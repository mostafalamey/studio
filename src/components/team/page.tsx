...
const TeamsManager: React.FC<{ users: AppUser[], teams: Team[] | undefined, teamsLoading: boolean, teamsError: Error | undefined, onSelectTeam: (team: Team) => void, isTeamChatActive: boolean, selectedTeamId: string | null }> = ({ users, teams, teamsLoading, teamsError, onSelectTeam, isTeamChatActive, selectedTeamId }) => {
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {teams?.map(team => (
                    <Card key={team.id} className={cn("hover:shadow-md transition-shadow cursor-pointer bg-border", { "bg-accent": isTeamChatActive && team.id === selectedTeamId })} onClick={() => onSelectTeam(team)}> {/* Changed to bg-border */}
                        <CardHeader>
...
const TeamMembersList: React.FC<{ team: Team, users: AppUser[], onSelectUser: (user: AppUser) => void, onSelectTeamChat: (team: Team) => void, isChatActive: boolean, selectedUserId: string | null }> = ({ team, users, onSelectUser, onSelectTeamChat, isChatActive, selectedUserId }) => {
    ...
    return (
        <div className="space-y-6">
             ...
             <Card className={cn("hover:shadow-md transition-shadow cursor-pointer bg-border", {"bg-accent": isChatActive && selectedUserId === team.id})} onClick={() => onSelectTeamChat(team)}> {/* Changed to bg-border */}
                 <CardHeader>
            ...
            <div className={cn(
                ...
                {teamMembers.map(member => (
                    <Card key={member.uid} className={cn("hover:shadow-md transition-shadow cursor-pointer bg-border", { "bg-accent": isChatActive && member.uid === selectedUserId })} onClick={() => onSelectUser(member)}> {/* Changed to bg-border */}
                        <CardHeader>
...
const EmployeeTeamView: React.FC<{ currentUser: AppUser, users: AppUser[], teams: Team[] | undefined, onSelectUser: (user: AppUser) => void, isChatActive: boolean, selectedUserId: string | null }> = ({ currentUser, users, teams, onSelectUser, isChatActive, selectedUserId }) => {
...
export default function TeamPage() {
    ...
    const [selectedUserId, setSelectedUserId] = useState<string | null>(null); // User/Team for highlighting selected card
    ...
    const handleSelectUser = (userToChat: AppUser) => {
        ...
        setSelectedUserId(userToChat.uid);
                         ...
                         {!selectedTeam && !selectedChatTarget && (
                             <>
                                 <TeamsManager
                                     ...
                                     isTeamChatActive={!!selectedChatTarget} // Pass chat state
                                     selectedTeamId={selectedUserId} // Pass selectedTeamId
                                 />
                                 {userRole === 'owner' && (
